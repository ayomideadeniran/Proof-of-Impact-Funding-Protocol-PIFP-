...
async fn execute_oracle_call(
    ...
    fn extract_tx_hash(output: &str) -> Option<String> {
        output
            ...
            .find_map(|line| {
                ...
                if trimmed.starts_with("transaction_hash:") {
                    return trimmed.split_once(':').map(|(_, value)| value.trim().to_string());
                }
                if trimmed.contains("\"transaction_hash\"") {
                    return trimmed
                        .split(':')
                        .nth(1)
    ...
    let sncast_account = std::env::var("ORACLE_SNCAST_ACCOUNT")
        .map_err(|_| "ORACLE_SNCAST_ACCOUNT not set".to_string())?;
    let calldata_args = calldata.iter().map(|felt| format!("{:#x}", felt)).collect::<Vec<_>>();
    let rpc_urls = vec![
        std::env::var("ORACLE_RPC_URL").unwrap_or_else(|_| "https://starknet-sepolia-rpc.publicnode.com".to_string()),
        std::env::var("ORACLE_RPC_URL_FALLBACK").unwrap_or_else(|_| "https://starknet-sepolia-rpc.publicnode.com".to_string()),
    ];
    ...
    for rpc_url in rpc_urls {
        let output = Command::new(&sncast_bin)
            .arg("--account")
            .arg(&sncast_account)
            .arg("--url")
            .arg(&rpc_url)
            ...
            .output()
            .await
            .map_err(|e| format!("Failed to start sncast: {e}"))?;

        if output.status.success() {
            let stdout = String::from_utf8_lossy(&output.stdout).to_string();
            if let Some(tx_hash) = extract_tx_hash(&stdout) {
                return Ok(tx_hash);
            }
        ...
        last_error = format!("rpc_url={rpc_url}; {stderr}");
    }
